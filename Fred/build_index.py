"""
build_index.py — Build FRED baseline dataset index (no image copying).
...
Run from ai_drone/Fred/:
    python build_index.py              # event mode
    python build_index.py --mode rgb   # RGB mode
"""
...
from config import DATA_FROM_FRED, SPLITS_YAML
...
def build(mode='event'):
    ...
    if not os.path.isfile(SPLITS_YAML):
        ...
        print("Run: python common/make_catalog.py --auto-split --train 70 --val 20 --test 10")
        return
...
# Generated by Fred/build_index.py --mode {mode}
