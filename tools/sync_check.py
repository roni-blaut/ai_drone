"""
sync_check.py â€” Verify synchronization between event frames and RGB frames.
...
Printed table columns:
  #        â€” frame pair index
  ev_t_s   â€” event frame time in seconds (from sequence start)
  rgb_t_s  â€” RGB frame time in seconds (relative, from first RGB frame)
  ev_cx/cy â€” drone bbox centre in event frame (normalised 0-1)
  rgb_cx/cyâ€” drone bbox centre in RGB frame   (normalised 0-1)
  Î”cx/Î”cy  â€” absolute difference â†’ near 0 = good sync
...
# â”€â”€ Helper: parse timestamps from filenames â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def _ev_ts_us(path):
    """Video_7_frame_9999000.txt  â†’  9999000 (Âµs)"""
    return int(os.path.basename(path).split('_frame_')[1].split('.')[0])

def _rgb_sec(path):
    """Video_7_17_01_52.636134.txt  â†’  seconds since midnight"""
...
# â”€â”€ Helper: load / draw YOLO labels â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Index Event frames â€” NUMERIC sort by Âµs timestamp â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
print(f"Event   : {len(ev_pairs)} frames   "
      f"t={ev_pairs[0][0]/1e6:.3f}s â€“ {ev_pairs[-1][0]/1e6:.3f}s")
...
# â”€â”€ Index RGB frames â€” alphabetical = chronological â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
print(f"RGB     : {len(rgb_pairs)} frames   "
      f"t=0.000s â€“ {rgb_pairs[-1][0] - rgb_t0:.3f}s (relative)")
print()


# â”€â”€ Pair by sequential index from start â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Both streams are ~30fps. Frame 0 = first frame in each stream (after --start).
# If synchronized, frame N in event â‰… frame N in RGB.

n_pairs = min(len(ev_pairs), len(rgb_pairs))
if args.n:
    n_pairs = min(n_pairs, args.n)

print(f"Showing {n_pairs} frame pairs  (event[i] â†” rgb[i])")
print()
...
EV_COLOR  = (0, 255, 255)    # cyan   â€” Event_YOLO boxes
RGB_COLOR = (0, 165, 255)    # orange â€” RGB_YOLO boxes
...
WIN = "Sync check â€” SPACE=next  â†/A=prev  â†’/D=+10  Q=quit"
...
while 0 <= idx < n_pairs:
    ...
    # â”€â”€ RGB panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Sync assessment â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if key in (ord('q'), 27):
        ...
        idx = max(0, idx - 1)
    elif key in (ord('d'), 83):          # D or RIGHT arrow â†’ jump +10
        idx = min(n_pairs - 1, idx + 10)
    else:                                # SPACE or anything else â†’ next
        idx += 1
