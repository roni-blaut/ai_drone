"""
build_dataset.py — Build YOLO training dataset from FRED sequences.
...
Single-sequence mode (legacy):
  ...
  Run: python build_dataset.py --single

For each 33ms window:
  1. Read events from events.raw (zip or folder)
  2. Apply noise filters
  3. Generate channels (config.N_CHANNELS: 3 or 4) → shape (N_CHANNELS, 720, 1280)
  4. Find matching annotation from coordinates.txt
  5. Save PNG (RGB for 3 channels, RGBA for 4) to dataset/images/
...
from config import (
    ...
    DATA_FROM_FRED, SPLITS_YAML,
    IMG_W, IMG_H, WINDOW_US, N_CHANNELS,
    TRAIN_RATIO, RANDOM_SEED,
    DEBUG_MODE, DEBUG_SAMPLES
)

_PNG_MODE = 'RGBA' if N_CHANNELS == 4 else 'RGB'
_CHANNEL_DESC = ('positive polarity, negative polarity, rotor map, time surface'
                 if N_CHANNELS == 4 else
                 'positive polarity, negative polarity, time surface')
...
def build_dataset(
    ...
    for t_start, events in reader.iter_windows(window_us, t_start=T_DRONE_START_US):
        ...
        img_path = os.path.join(output_dir, 'images', split,
                                frame_name + '.png')
        Image.fromarray(img_hwc, mode=_PNG_MODE).save(img_path)
...
def _write_yaml(output_dir):
    ...
    yaml_str = f"""# FRED {N_CHANNELS}-Channel Drone Detection Dataset
...
# {N_CHANNELS}-channel input: {_CHANNEL_DESC}
channels: {N_CHANNELS}
...
def check_dataset(output_dir=DATASET_DIR):
    ...
    if not os.path.isdir(img_dir):
        print("  ERROR: images/ not found — run build_dataset.py first.")
        return
...
def _ensure_zip(seq_num, catalog, data_dir):
    ...
    if not fid:
        raise FileNotFoundError(
            ...
            f"  python common/make_catalog.py --scan-drive"
        )
...
def _write_yaml_multi(output_dir):
    ...
    abs_dir  = os.path.abspath(output_dir)
    yaml_str = f"""# FRED {N_CHANNELS}-Channel Drone Detection Dataset
# Generated by build_dataset.py (multi-sequence mode)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python build_dataset.py                 # build from splits.yaml\n"
            "  python build_dataset.py --download      # auto-download missing zips\n"
            "  python build_dataset.py --check         # verify existing dataset\n"
            "  python build_dataset.py --single        # legacy seq-7 only mode\n"
        ),
    )
